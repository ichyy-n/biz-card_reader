...
- 管理者API（ユーザー承認・取消・一覧）
- Google SpreadsheetをデータソースとしてGoogle AppSheetでUIアプリを構築可能
- AppSheetを使うと名刺データの閲覧・検索・フィルタリングがスマホから可能
- 画像サイズ制限（5MB）
- トークンの暗号化保存（Fernet）
...
Google Sheets (データソース)
  │
  └── Google AppSheet (UIアプリ)
        └── 名刺データの閲覧・検索・フィルタリング（スマホ対応）
```
...
- **ホスティング**: PaaS（Render, Railway, Fly.io等）または自前サーバー
...
FastAPIアプリのため、uvicornが動作するPaaS（Render, Railway, Fly.io等）または自前サーバーにデプロイ可能。以下はRenderを例とした手順。
